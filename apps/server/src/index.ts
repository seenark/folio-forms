...
function callbackDocumentUrl(value: unknown): string | null {
  if (typeof value !== "string") {
    return null;
  }
  try {
    ...
    if (
      ...
      !callbackOrigins.has(url.origin)
    ) {
      return null;
    }
    if (
...
function launchForceSave(operation: typeof operations.$inferSelect): void {
  void (async () => {
    try {
      ...
      const hasChanges = await forceSave(
...
async function completeSubmitOperation(
  ...
  const pdf = await convertDocxToPdf(documentKey);
...
async function finalizeCallback(
  ...
  const metadata = operationMetadata(operation.metadata);
  const callbackUrl = callbackDocumentUrl(payload.url);
  if (
...
const app = new Elysia()
  ...
  .post("/api/admin/forms", async ({ request, body }) => {
    ...
    return { form: formSummary(form), templateAvailable: Boolean(sourcePath) };
  })
  ...
  .post("/api/admin/forms/:id/save", async ({ request, params, body, set }) => {
    const identity = await requireIdentity(request);
    requireAdmin(identity);
    const form = await findFormById(params.id);
    const input = asRecord(body);
    ...
    if (!form.templateDraftKey || form.templateDraftKey !== documentKey) {
      fail(409, "stale_document", "The editor document is no longer current");
    }
    if (
  ...
  .post("/api/forms/:publicId/start", async ({ request, params }) => {
    ...
    if (existing && existing.status === "submitting") {
      fail(409, "operation_in_progress", "Your submission is being processed");
    }
    if (
      existing &&
    ...
    async ({ request, params, body, set }) => {
      ...
      const operation = await createOperation({
        ...
        metadata: {
          ...
          publicId: form.publicId,
          responseId: response.id,
          stagedDocxPath: artifactPath("operations", operationId, "draft.docx"),
        },
        responseId: response.id,
  ...
  .post(
    ...
    async ({ request, params, body, set }) => {
      ...
      const metadata: OperationMetadata = {
        ...
        finalDocxPath: artifactPath("submissions", submissionId, "filled.docx"),
      ...
      if (!operation) {
        fail(500, "operation_failed", "Unable to create submission operation");
      }
      set.status = 202;
      launchForceSave(operation);
      return {
  ...
  .get("/onlyoffice/document/:key", async ({ params, query, set }) => {
    ...
    if (!verifyDocumentAccessToken(token, key)) {
      fail(401, "unauthorized", "Document access token is invalid or expired");
    }
  ...
  .post("/onlyoffice/callback", async ({ body }) => {
    ...
    try {
      await finalizeCallback(operationId, payload);
      return { error: 0 };
    } catch (error) {
      const metadata = operationMetadata(operation.metadata);
      await updateOperationFailed(operation.id, errorMessage(error));
