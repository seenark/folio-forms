...
The same mock polls `POST /status` with only the External Reference; Folio returns only `pending`, `draft`, `submitted`, `deleted`, or `expired` plus lifecycle timestamps and latest Correction number. Status failures and unknown references are non-enumerating. Pending or reserved Handoffs are swept every 60 seconds after expiry: claim, code digest, Prefill, and normalized identity are purged while the lookup digest and timestamps remain.
